function convert()
{
  ...
  else
  {
    ...
    .then(response =>{
       var result = eval("response.rates." + currency2) * amount;
       document.querySelector('.conversion-result').innerHTML = todayD + " (dnes): " + amount + " " + currency1 + " = " + roundNumber(result) + " " + currency2;
    ...
    else
    {
      ...
      fetch('https://api.exchangeratesapi.io/' + formattedDate + '?base=' + currency1)   
      .then(response => {
        return response.json();
     })
    .then(response =>{
   ...
   fetch('https://api.exchangeratesapi.io/history?start_at=' + formatDate(weekAgo) + '&end_at=' + formatDate(today) + '&base=' + currency1)
   .then(response => {
       return response.json();
    })
   .then(response =>{
...
function createGraph(response)
{
  ...
  var amount =  document.getElementById('amount').value;
    ...
    var dateResult;
    if(eval("response.rates" + "[\"" + formatDate(yesterday) + "\"]") != undefined)
    {
      dateResult = amount * eval("response.rates" + "[\"" + formatDate(yesterday) + "\"]" + "." + currency2);
      lastAmount = dateResult;
    }
    else
    {
      dateResult = lastAmount;
    }
    ...
    myData.reverse();
      ...
      myChart = new Chart(document.getElementById('line-chart'), {
        ...
        data: {
          ...
          datasets: [ { 
              data: myData,
              label: "Hodnota " + amount + " " + currency1 + " v " + currency2,
              borderColor: "#3cba9f",
              fill: false
            }, 
          ]
